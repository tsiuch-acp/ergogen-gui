      ...
      onPath: wp => {
        ...
        switch (p.type) {
          ...
          case 'arc':
            ...
            const arc_mid = m.point.rotate(m.point.add(arc_center, [p.radius, 0]), angle_start + (angle_diff / 2), arc_center)
            const arc_end = m.point.rotate(m.point.add(arc_center, [p.radius, 0]), angle_start + angle_diff, arc_center)
            grs.push(`(gr_arc (start ${xy(arc_start)}) (mid ${xy(arc_mid)}) (end ${xy(arc_end)}) (layer ${layer}) (width 0.15))`)
            break
          case 'circle':
